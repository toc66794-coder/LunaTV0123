...
- �🚀 **播放体验增强**：优化自动全屏触发逻辑，实现平滑亮度调节。
- 👆 **交互冲突修复**：解决长按三倍速与滑动調節（亮度/音量/进度）的手势冲突。
- 🔋 **省电管理**：支持 Screen Wake Lock API，播放期間防止手機自動熄屏。
- 🛡️ **流媒體兼容**：支持 M3U8 代理模式，有效解決部分源的跨域與 Referer 限制問題。
- 👆 **智慧手勢控制**：
  - **雙擊**：左側 1/3 快退 10 秒 / 右側 1/3 快進 10 秒 / 中間 暫停播放。
  - **單擊**：喚醒控制列並於左上角顯示詳細資訊（片名、集數、當前時間）。
  - **長按**：3 倍速播放（支援防誤觸鎖定）。
  - **滑動**：左側調整亮度 / 右側調整音量。
- 🧪 **投屏省電 (實驗性)**：內置基於 DOM 的「零渲染」省電遮罩層（底層架構已完成）。
...
MoonTV 提供了專為電視大螢幕設計的沈浸式介面，支援完整的遙控器操作。

### 🎮 遙控器與鍵盤對照表

| 遙控器按鍵         | 電腦鍵盤      | 播放時功能                 | 介面導航功能 |
| ------------------ | ------------- | -------------------------- | ------------ |
| **OK / Enter**     | Enter / Space | 播放 / 暫停 (長按重置速度) | 確認 / 選擇  |
| **Menu**           | M             | **開啟設定選單**           | -            |
| **Direction Keys** | 方向鍵        | 快進 / 快退 / 調整速度     | 移動焦點     |
| **Back / Return**  | Esc           | 退出播放                   | 返回上一頁   |
| **Channel +/-**    | PageUp / Down | 上一集 / 下一集            | -            |

### ✨ 獨家功能

1.  **智慧資源選擇**

    - 系統會自動過濾無效資源，優先播放可用的高清線路。
    - **換源功能**：在影片詳情頁底部點擊「換源」按鈕，可手動切換至其他片源。

2.  **片頭片尾自動跳過**

    - 按下 **Menu** 鍵開啟設定選單。
    - 選擇「設定片頭結束點」或「設定片尾開始點」。
    - 系統會自動記憶該系列影片的時間點，實現無縫追劇體驗。

3.  **視覺化集數與速度控制**
    - **集數選擇**：在設定選單中可開啟視覺化集數列表，支援橫向快速瀏覽。
    - **變速播放**：使用遙控器上下鍵可快速調整播放速度 (0.5x ~ 3.0x)。
...
- [ ] **演員關鍵字搜尋**：規劃於切換至 Docker/VPS 架構後開發，利用本地索引（Kvrocks/SQLite）解決聚合搜尋效能問題。
- [ ] **訂閱功能**：支援導入第三方訂閱源。
- [ ] **UI 換膚服務**：支援多套主題切換。
